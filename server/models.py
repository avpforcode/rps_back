...
class Arena:
  ...
  def get_rivals(self, gamer):
    rival = lambda g: g is not gamer and g.ready and g.game_type == gamer.game_type
  ...
  async def broadcast(self, recipients = None, exclude = None):
    ...
    for player in players:
      if exclude and player in exclude: continue
...
class Game:
  ...
  def throw(self, user, value):
    ...
    if user.uid in self.throws: return
  ...
  def two_players(self):
    ...
    if winning_throw == TIE: self.winner = TIE
  ...
  def play(throws):
    ...
    if len(throws) == 1: return TIE

    if PASS in throws: throws.remove(PASS)
    if len(throws) == 1: return throws[0]
    if len(throws) == 3: return TIE
